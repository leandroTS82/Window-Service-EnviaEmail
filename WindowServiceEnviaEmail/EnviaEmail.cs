    ...
    public static class ServicoEmail
    {
        public static bool EnviaEmail(string destinatarios, string assunto, string mensagem, out string exception)
        {
            try
            {
                ...
                {
                    mess.CC.Add(new MailAddress(cc));
                }
                string[] arrDestinatarioscopiaOculta = destinatarios.Split(';');
                foreach (string bcc in arrDestinatarioscopiaOculta)
                {
                    mess.Bcc.Add(new MailAddress(bcc)); 
                }
